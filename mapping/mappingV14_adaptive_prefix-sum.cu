...
__global__ void restore_signal(int* d_output_lengths, uint8_t* d_concatenated_signals, uint8_t* d_signals, int data_size, int last_length)
{
    ...
    // int length = d_output_lengths[warp];
    int length = warp == (gridDim.x - 1) ? last_length : d_output_lengths[warp + 1] - d_output_lengths[warp];
...
__global__ void map_values_kernel_16b_thrust(const uint16_t* data, uint8_t* code, uint8_t* bit_signal, uint16_t* centers, int* signal_length ,int data_size, int shift) 
{
    const int tid = threadIdx.x;
    const int bid = blockIdx.x;
    const int idx = bid * blockDim.x + tid;
...
__global__ void map_values_kernel_16b_decoupled(
    const uint16_t* data, 
    uint8_t* code, 
    uint8_t* bit_signal, 
    uint16_t* centers, 
...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < 0; i++)
    {
        if (data_type == "uint16" && gsize <= 1024) {
            map_values_kernel_16b_decoupled<<<gsize, bsize>>>(
    ...
    if(data_type == "uint16" && gsize <= 1024)
    {
        ...
        map_values_kernel_16b_decoupled<<<gsize, bsize, sizeof(unsigned int)*2, stream>>>(
    ...
    else if(data_type == "uint16" && gsize > 1024)
    {
        printf("\033[0;36m=======>Use Thrust Prefix-sum<=======\033[1m\n") ;
        //adm
        cudaEventRecord(start);
        map_values_kernel_16b_thrust<<<gsize, bsize>>>(
    ...
    // ----------------Decmp------------------
    // decmp kernel 1: restore
    uint8_t* d_signals;
    cudaMalloc(&d_signals, bsize * gsize * cmp_chunk * sizeof(uint8_t));
    cudaEventRecord(start);
    restore_signal<<<gsize, bsize>>>(d_output_lengths, d_concatenated_signals, d_signals, num_elements, h_signal_length[gsize - 1]);
    cudaDeviceSynchronize();
    ...
    float total_decmp_throughput = (data_size / 1024.0 / 1024 / 1024) / ((restore_milliseconds + decmp_adm_milliseconds) / 1000.0f); 
    ...
    printf("Total cost %.2f ms\n",(restore_milliseconds + decmp_adm_milliseconds));
