...
static const int max_signals_32b = 3;  
static const int decmp_chunk = 2;  // one decomp thread process decmp_chunk cmp thread

__global__ void decompress_kernel_16b(uint8_t* compressed_data, const int* offsets, const int* output_lengths, uint16_t* decompressed_data, int center, int shift, uint8_t signal) {
...
__global__ void decompress_kernel_32b(uint8_t* compressed_data, const int* offsets, const int* output_lengths, uint32_t* decompressed_data, int center, int shift, uint8_t signal) {
    const int block_id = blockIdx.x * blockDim.x + threadIdx.x;  // 每个线程对应一个 block
...
__global__ void map_values_kernel_16b(const uint16_t* data, uint8_t* result, const int* offsets, int* output_lengths, int data_size, int center, int shift, uint8_t signal) 
{
...
__global__ void map_values_kernel_32b(const uint32_t* data, uint8_t* result, const int* offsets, int* output_lengths, int data_size, int center, int shift, uint8_t signal) 
{
    const int tid = threadIdx.x;
    const int bid = blockIdx.x;
    const int idx = bid * blockDim.x + tid;
...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < 2; i++)
    {
        if (data_type == "uint16") {
            map_values_kernel_16b<<<gsize, bsize>>>(
                (uint16_t*)d_data, d_result, nullptr, d_output_lengths, num_elements, center, shift, signal);
        } else if (data_type == "uint32") {
            map_values_kernel_32b<<<gsize, bsize>>>(
                (uint32_t*)d_data, d_result, nullptr, d_output_lengths, num_elements, center, shift, signal);
        }
    }
    ...
    if (data_type == "uint16" && has_center == false) {
        center = compute_mean<uint16_t>(d_data, num_elements);
    ...
    if (data_type == "uint16") {
        decompress_kernel_16b<<<gsize2, bsize>>>(d_output, d_offsets, d_output_lengths, (uint16_t*)d_decmpdata, center, shift, signal);
    } else if (data_type == "uint32") {
        decompress_kernel_32b<<<gsize2, bsize>>>(d_output, d_offsets, d_output_lengths, (uint32_t*)d_decmpdata, center, shift, signal);
    }
