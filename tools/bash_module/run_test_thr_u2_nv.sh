        ...
        if [[ -d $TEST_DIR_u2"/"$dir ]]; then
            ...
            do
                if [[ $file == *".u2" ]]; then
                    ...
                    # Step 3: ans
                    ans_output=$($ANS_cmp_nv "$file_path" "$file_path.tmp")
                    ans_cmp_time=$(echo "$ans_output" | grep "comp   time" | awk 'NR==1 {print $3}')
                    ans_output=$($ANS_decmp_nv "$file_path.tmp" "$file_path.out")
                    ...
                    ans_cmp_time=$(echo "$ans_output" | grep "comp   time" | awk 'NR==1 {print $3}')
                    ans_output=$($ANS_decmp_nv "$adm_output_path.tmp" "$adm_output_path.out")
                    ans_decmp_time=$(echo "$ans_output" | grep "decomp time" | awk 'NR==1 {print $3}')
