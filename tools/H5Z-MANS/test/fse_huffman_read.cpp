...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) std::printf("[fse_huffman_read] %s\n", msg);
    };
    ...
    char in[64];
    std::snprintf(in, sizeof(in), "datasets/fse_huffman_rank%d.h5", rank);
    ...
    if (rank == 0) {
        ...
        std::printf("fse_huffman_read ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s\n",
