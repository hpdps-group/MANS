...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) std::printf("[none_write] %s\n", msg);
    };

    stage("loading input");
    char in[64], out[64];
    std::snprintf(in, sizeof(in), "rank%d.bin", rank);
    std::snprintf(out, sizeof(out), "rank%d.none.h5", rank);
    ...
    hsize_t chunk[1] = {kChunkBytes / (hsize_t)sizeof(std::uint16_t)};
    ...
    if (rank == 0) {
        ...
        std::printf("none_write ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s comp_ratio=%.2fx\n",
