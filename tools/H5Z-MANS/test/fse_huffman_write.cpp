...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) std::printf("[fse_huffman_write] %s\n", msg);
    };

    stage("loading input");
    char in[64], out[64];
    std::snprintf(in, sizeof(in), "datasets/rank%d.bin", rank);
    std::snprintf(out, sizeof(out), "datasets/fse_huffman_rank%d.h5", rank);
    ...
    if (rank == 0) std::printf("[fse_huffman_write] chunk-size-mb=%.6g\n", chunk_size_mb);
    ...
    if (rank == 0) {
        ...
        std::printf("fse_huffman_write ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s comp_ratio=%.2fx\n",
