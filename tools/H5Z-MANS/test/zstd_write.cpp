...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) std::printf("[zstd_write] %s\n", msg);
    };
    ...
    if (rank == 0) std::printf("[zstd_write] chunk-size-mb=%.6g\n", chunk_size_mb);
    ...
    if (rank == 0) {
        ...
        std::printf("zstd_write ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s comp_ratio=%.2fx\n",
